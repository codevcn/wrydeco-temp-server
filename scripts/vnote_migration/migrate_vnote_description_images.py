...
# Chỉ migrate URL xuất hiện BÊN TRONG thẻ <img> và hostname phải
# CHÍNH XÁC là vnote.io.vn. Không giới hạn attribute: src, data-src,
# srcset hoặc attribute khác đều được xử lý nếu chính URL có exact host này.
# URL vnote.io.vn nằm ngoài <img>, subdomain và domain khác đều skip.
TARGET_IMAGE_DOMAIN = "vnote.io.vn"
...
# Lấy từng URL token bên trong chính thẻ <img>. Sau đó hostname được
# kiểm tra bằng urlparse, vì vậy regex này không tự quyết định domain target.
...
def create_download_session():
    """Retry chỉ cho GET ảnh nguồn Vnote, không retry các POST tạo asset."""
...
def normalize_vnote_url(value: str):
...
def is_vnote_img_src_url(value: str):
    normalized = normalize_vnote_url(value)
    ...
    return (
        ...
        and host == TARGET_IMAGE_DOMAIN
    )


def download_vnote_image(
    session: requests.Session,
    vnote_url: str,
    temp_dir: Path,
):
    request_url = normalize_vnote_url(vnote_url)
    ...
    response = session.get(
        ...
        stream=True,
        timeout=(15, 90),
        headers={"Referer": "https://vnote.io.vn/"},
    )
    ...
    if not content_type.startswith("image/"):
        raise RuntimeError(
            "vnote.io.vn không trả về image.\n"
            f"URL: {request_url}\n"
            f"Content-Type: {content_type}"
        )
    ...
    filename = f"vnote-rich-{url_hash}{ext}"
...
def migrate_image(
    ...
    # Dùng URL đã HTML-unescape/protocol-normalize làm cache key để
    # cùng một ảnh không upload lại chỉ vì cách viết URL khác nhau.
    cache_key = normalize_vnote_url(vnote_url)
    ...
    try:
        local_path, content_type = download_vnote_image(
            download_session,
            vnote_url,
            temp_dir,
        )
...
def extract_img_url_occurrences(value: str):
    """Trả về mọi URL token nằm trong thẻ <img>, giữ nguyên thứ tự."""
    if not value:
        return []
    ...
    for tag_match in IMG_TAG_RE.finditer(value):
        ...
        urls.extend(m.group("url") for m in IMG_URL_RE.finditer(tag))

    return urls


def find_vnote_img_url_occurrences(value: str):
    """Chỉ URL trong <img> có hostname chính xác vnote.io.vn."""
    return [
        url
        for url in extract_img_url_occurrences(value)
        if is_vnote_img_src_url(url)
    ]


def find_vnote_img_urls(value: str):
    # Deduplicate trong cùng Rich Description nhưng giữ nguyên thứ tự.
    return list(dict.fromkeys(find_vnote_img_url_occurrences(value)))


def replace_vnote_img_urls(value: str, replacements: dict):
    """
    Chỉ rewrite URL token nằm bên trong thẻ <img> khi exact hostname là
    vnote.io.vn. Attribute có thể là src, data-src, srcset hoặc attribute khác.

    Không sửa:
    - URL vnote.io.vn nằm ngoài <img>
    - URL domain khác nằm trong <img>
    - subdomain như cdn.vnote.io.vn
    """
    ...
    def replace_tag(tag_match):
        ...
        def replace_url(url_match):
            ...
            if not is_vnote_img_src_url(original_url):
                return original_url

            return replacements.get(original_url, original_url)

        return IMG_URL_RE.sub(replace_url, tag)

    return IMG_TAG_RE.sub(replace_tag, value)
...
def process_csv(
    ...
    # HARD LOCK amazon_link như flow cũ.
    amazon_link_before = [
        row.get(AMAZON_LINK_COLUMN, "")
        for row in rows
    ]

    # Snapshot toàn bộ URL bên trong <img> trước khi xử lý. Validation cuối sẽ
    # đảm bảo mọi URL KHÔNG thuộc vnote.io.vn vẫn giữ nguyên đúng vị trí/giá trị.
    img_srcs_before = [
        extract_img_url_occurrences(row.get(RICH_DESCRIPTION_COLUMN, "") or "")
        for row in rows
    ]
    ...
    for row in rows:
        ...
        urls = find_vnote_img_url_occurrences(rich)

        if urls:
            total_occurrences += len(urls)
            unique_urls.update(normalize_vnote_url(url) for url in urls)
    ...
    print("=" * 70)
    print("VNOTE.IO.VN URLS INSIDE <IMG> -> SHOPIFY FILES MIGRATION")
    print("=" * 70)
    ...
    print(f"Vnote img URLs    : {total_occurrences}")
    print(f"Unique images     : {len(unique_urls)}")
    print("Target rule       : URL inside <img>, hostname == vnote.io.vn ONLY")
    print("=" * 70)
    ...
    with tempfile.TemporaryDirectory(
        ...
        for product_number, (handle, row_indexes) in enumerate(
            ...
            for row_index in row_indexes:
                ...
                vnote_urls = find_vnote_img_urls(original_rich)
                ...
                for image_number, vnote_url in enumerate(
                    vnote_urls,
                    start=1,
                ):
                    print(
                        f"   Image {image_number}/{len(vnote_urls)}"
                    )
                    ...
                    replacements[vnote_url] = public_url
                    migrated_unique_urls.add(normalize_vnote_url(vnote_url))

                # CỰC KỲ QUAN TRỌNG: không dùng str.replace toàn HTML.
                # Chỉ rewrite URL target nằm bên trong thẻ img.
                new_rich = replace_vnote_img_urls(
                    original_rich,
                    replacements,
                )
    ...
    if remaining:
        print(
            "[ERROR] Vẫn còn URL vnote.io.vn bên trong thẻ <img>:"
        )
    ...
    print("[OK] Không còn URL exact domain vnote.io.vn bên trong <img>.")

    # Validate URL domain khác bên trong <img> skip 100%.
    for zero_index, row in enumerate(rows):
        after_urls = extract_img_url_occurrences(
            row.get(RICH_DESCRIPTION_COLUMN, "") or ""
        )
        before_urls = img_srcs_before[zero_index]

        if len(after_urls) != len(before_urls):
            raise RuntimeError(
                "CRITICAL ERROR: Số lượng URL bên trong <img> bị thay đổi ở CSV row "
                f"{zero_index + 2}."
            )

        for img_index, original_url in enumerate(before_urls):
            if is_vnote_img_src_url(original_url):
                continue

            if after_urls[img_index] != original_url:
                raise RuntimeError(
                    "CRITICAL ERROR: URL domain khác bên trong <img> đã bị thay đổi.\n"
                    f"CSV row: {zero_index + 2}\n"
                    f"URL index: {img_index + 1}\n"
                    f"Before: {original_url}\n"
                    f"After : {after_urls[img_index]}"
                )

    print("[OK] Tất cả URL domain khác bên trong <img> được skip/giữ nguyên 100%.")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Chỉ download URL nằm trong <img> có exact domain vnote.io.vn, "
            "upload vào Shopify Content > Files, chờ Shopify CDN READY, "
            "rồi replace đúng URL đó. Domain khác và URL ngoài <img> được skip."
        )
    )
    ...
    parser.add_argument(
        ...
        "--output",
        help=(
            "Output CSV. Default: <input>_vnote_shopify_files.csv"
        ),
    )
    ...
    if args.output:
        ...
        output_path = (
            input_path.parent
            / f"{input_path.stem}_vnote_shopify_files.csv"
        )
    ...
    if args.cache:
        ...
        cache_path = (
            input_path.parent
            / f"{input_path.stem}_vnote_shopify_upload_cache.json"
        )
