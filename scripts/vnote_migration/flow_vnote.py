...
MIGRATE_SCRIPT = "migrate_vnote_description_images.py"
...
def output_path_for(csv_path: Path):
    return csv_path.parent / f"{csv_path.stem}_vnote_shopify_files.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "3. Chỉ upload <img src> có domain vnote.io.vn vào Shopify "
            "Content > Files\n"
            "4. Replace đúng src đó bằng Shopify CDN URL; domain khác skip"
        ),
    ...
    # .upload.env chỉ cần khi có ảnh Vnote cần migrate, nhưng kiểm tra
    ...
    print("=" * 80)
    print("WRYDECO VNOTE -> SHOPIFY FILES FLOW")
    print("=" * 80)
    ...
    # ============================================================
    # Dùng 1 cache chung cho toàn flow để cùng một Vnote URL không bị
    # upload trùng nếu xuất hiện ở nhiều part/product.
    shared_cache = (
        input_csv.parent
        / f"{input_csv.stem}_vnote_shopify_upload_cache.json"
    )
    ...
    print()
    print(
        "Safety: amazon_link vẫn hard-lock; chỉ src của <img> có exact "
        "domain vnote.io.vn được replace; mọi domain khác được skip."
    )
